...
# /Users/ali/Projects/foo/bar/baz -> f/b/baz (if >3 segments or >20 chars)
abbreviate_path() {
    local path="$1"
    local segments
    ...
    if [[ ${#path} -le 20 && $count -le 3 ]]; then
        echo "$path"
        return
    fi

    # Abbreviate: keep last segment full, abbreviate others to first char
    local result=""
